...
bool    ImFontAtlas::Build()
{
    ...
    // [Bruno Levy] 05/18/2016 removes a 'potentially uninitialized var.' warning.
    memset(&spc, 0, sizeof(stbtt_pack_context));
    ...
    for (int input_i = 0; input_i < ConfigData.Size; input_i++)
    {
        ...
        for (int i = 0; i < tmp.RangesCount; i++)
        {
            ...
            for (int char_idx = 0; char_idx < range.num_chars; char_idx += 1)
            {
                ...
                // [Bruno Levy] 01/30/2017 Seems to fix Valgrind UMR and wrecked fonts with Intel C compiler
		// when compiling in dynamic mode (but bug is still there in static mode, to be further investigated...)
		memset(&glyph, 0, sizeof(ImFont::Glyph));
