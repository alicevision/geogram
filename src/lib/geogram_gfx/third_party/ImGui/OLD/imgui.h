...
public:
	...
	int rhs_size = rhs.Size; rhs.Size = Size; Size = rhs_size; int rhs_cap = rhs.Capacity; rhs.Capacity = Capacity; Capacity = rhs_cap;
	value_type* rhs_data = rhs.Data; rhs.Data = Data; Data = rhs_data;
    }
	...
	IM_ASSERT(it >= Data && it < Data+Size); const ptrdiff_t off = it - Data;
	memmove(Data + off, Data + off + 1, ((size_t)Size - (size_t)off - 1) * sizeof(value_type)); Size--; return Data + off;
    }
	...
	Data[off] = v; Size++; return Data + off;
    }
};
...
struct ImGuiListClipper
{
    ...
    void Begin(int count, float height)        // items_height: generally pass GetTextLineHeightWithSpacing() or GetItemsLineHeightWithSpacing()
    {
        ...
        ImGui::SetCursorPosY(ImGui::GetCursorPosY() + float(DisplayStart) * ItemsHeight);    // advance cursor
    }
    void End()
    {
        IM_ASSERT(ItemsCount >= 0);
        ImGui::SetCursorPosY(ImGui::GetCursorPosY() + float(ItemsCount - DisplayEnd) * ItemsHeight); // advance cursor
        ItemsCount = -1;
    }
};
